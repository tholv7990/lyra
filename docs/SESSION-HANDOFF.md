...
## ✅ Shipped this session (all live on dev; commits in order)
1. **Viewer Crawler access** (`505881ac`): dropped `@RequireCreate()` from connectors `resolve`/`download` (kept `publish` gated) — they forward no metered key; Viewers can use the Crawler. Security-reviewed SECURE.
2. **Multi-provider run fallback** (`0d9ffdb1`): a text/brain pipeline step that hits a **retryable** error (429/5xx/timeout/quota) auto-retries on the other text providers the workspace has BYO keys for. `fallbackChain` (shared) + `RunsService.executeWithFallback` + `isRetryableProviderError`. Invariant 7 preserved; primary-only cache write. Anthropic/OpenAI/DeepSeek order.
3. **Product-research v1 spine** (`5990ddd1`): `Product` entity — **project-owned** (`projectId`, peer of Task), api module mirroring Task (`/projects/:id/products`), cascade-delete. `@lyra/shared`: `ProductStatus`, evidence/economics/scoring types, `WEIGHTS`/`SubScores`, pure fns `computeUnitEcon`/`weightedScore`/`gradeConfidence`/`decide` (Vitest).
4. **Agentic research provider** (`7bf71cf4`): `ResearchStepProvider` — Tavily search (BYO key via connector-credentials, `SearchBackend` interface → Exa-swappable) + a **bounded agent loop** (plan→search→extract→reflect→≤2 rounds, hard caps) with **grounding enforced in CODE** (claims must cite a real fetched `SourceRow` or are dropped; `accessDate` code-stamped; SSRF-safe dead-link sweep). NOT yet registered (was wired in 3a).
5. **3a engine enablers** (`47f816d7`): the **evidence ledger** (`RunStep`/`Step` += `evidence?/sources?/data?`; `assembleLedger` → read-only `RunLedger` in each step's context; `toStep` maps them so they survive reload); **`Provider.Research` registered** (ProviderRegistry → `Partial`+guarded; research LLM upgraded to Anthropic→OpenAI→DeepSeek); CODE-as-Action providers `unit-econ`/`evaluate` + **`SaveProduct`** (ledger → Product).
6. **3b lean pipeline template** (`02520fe2`): idempotent **`POST /workspaces/:id/pipelines/seed-research`** (`ResearchTemplateService`) creates a 6-step "Product research" Pipeline (Research → DemandGate → Score → UnitEcon → Evaluate → Save@gate). New: **`ScoreAction`** (LLM-backed grounded subScores producer — the missing piece), **`DemandGate`** (fails <3 distinct-source demand signals) + `demandSignalCount`. End-to-end chain test proves it (evidence → score 80 → TEST_NOW → saved Product).
7. **3c-1 UI** (`d4d5a46a`): **Tavily key card** (Settings, mirrors PostizKeySection + a `GET credentials/tavily` route), **"Create research pipeline"** button (Pipelines), **Products board + `Tasks|Products` toggle + evidence-ledger detail modal** (ProjectDetail; status via MenuPicker + outcome edit). Also fixed a **latent web build break** (`Provider.Research` missing from `PROVIDER_LABELS`, dormant since 3a).
8. **Mobile toggle-overflow hotfix** (`efd41ed7` → `6addbaea`): the `Tasks|Products` toggle (full-width on mobile) shared its row with the New-task button → horizontal overflow on iOS (Chromium *clipped* it, hiding it from my probe). Fix: the row **wraps** — toggle full-width, New-task button on its own line below. Verified 0 overflow at 360/390/440px.

## ⚠️ Deploy / verify specifics
- **Deploy** = ff `codex-dev` ← feat branch → (api/shared) `pnpm --filter @lyra/shared build && --filter @lyra/api build` + `pm2 restart lyra-api` + boot-verify ("Nest application successfully started") → `git push origin codex-dev:dev`. Web-only → HMR (no restart).
- **Worktree e2e gotcha:** a fresh worktree lacks the gitignored `.env`, so e2e fails `ENCRYPTION_KEY must be 64 hex` — set `$env:ENCRYPTION_KEY` to a 64-hex string first (`worktree-e2e-encryption-key` memory). With it, e2e = 84/84.
- **Mobile verify:** check **390px AND ~440px** (Pro-Max) and **every tab/state** — a control rendered only in one state (the New-task button on the Tasks tab) can overflow. Chromium can *clip* an overflow iOS scrolls → inspect element `getBoundingClientRect().right`, not just `docScroll` (`verify-mobile-not-just-desktop` memory).

## 🔜 Pending / open (don't start unless asked)
- **3c-2 (deferred UI polish):** group the run's steps into a **4-phase view** (Find · Validate · Economics · Decide) in `RunFlow`; a tailored **research-run form** (nicer than the raw variable inputs).
- **Live Tavily smoke STILL PENDING:** everything is mock/unit-verified; **no real run has happened** (needs a **Tavily key + an AI key** on a workspace → add via Settings → run the seeded "Product research" pipeline against a task → approve the save-gate → a real graded Product). This is the make-or-break grounding proof.
- **Spec + plan docs are UNCOMMITTED** (untracked, per convention): `docs/superpowers/{specs,plans}/2026-06-22-provider-fallback*`, `…/2026-06-23-product-research-spine*`, `…/2026-06-23-agentic-research-provider*`, `…/2026-06-23-research-pipeline-3a*`, `…/-3b*`, `…/-3c1*`. Commit when asked. **This handoff entry is also uncommitted.**
- **Full §1C.1 fidelity (later):** `resolve-inputs` + `demand-gate`(richer) + `competition`(fan-out) + `ground&dedupe` steps; `runScenarios`; `Run.productId?`/`Task.productId?`/`PublishedPost.productId?` + monitor `productId` back-refs.
- **Parked thread:** post-render asset-QA / self-review (approach signed off, no spec) — `[[product-research-spine-shipped]]` notes it.
...
## ✅ Shipped this session (live on dev)
1. **Bell + Settings-provider redesigns + User Requests module** (earlier): portaled bell popup w/ rich cards + 5/page; Settings is now provider **cards** (add-key modal, modality icons text/image/audio/video reusing `promptType.tsx`, collapsible, createdBy/createdAt) + a curated catalog (3 available / 7 coming-soon); a generic **User Requests** subsystem (`apps/api/src/requests`; shared `RequestType`/`RequestStatus`; `UserRequest` w/ vote-ready `voters[]`) — provider-request submit from Settings + an Admin **Requests** tab. Provider/request modals use the shared `.dialog`/`.field` pattern + a reusable `.dialog-close` X.
2. **Change 1 — workspace personal/team + admin-gated team upgrade.** `WorkspaceType` enum; **invites/Members gated to `team`**; free team-create (`POST /workspaces` + switcher "+add team") **retired**; a `team-upgrade` request (reuses Requests) that the admin **resolves → flips `Workspace.type` to team** (`WorkspacesService.upgradeToTeam` — one-way, idempotent, writes only `type`). Members page shows an **upgrade prompt** on personal. Spec `…workspace-personal-team-model-design.md`, plan `…workspace-type-team-upgrade.md`.
3. **Task layer (Plan 2) — project → tasks → pipelines.** New **`Task`** collection (`apps/api/src/tasks`); `TaskStatus` (new·in_progress·on_hold·complete, **manual**); single **assignee** (member-validated, hidden on personal). **`project.pipelines` REMOVED → `task.pipelines`**; **`Run.taskId`**; run routes re-rooted under **`/projects/:id/tasks/:taskId/...`**. Web: **ProjectDetail = task board** (`TaskBoard` + `lib/taskStatus.ts`); new **`TaskDetail.tsx`** = the run workbench scoped to a task; route `/projects/:id/tasks/:taskId`. **Migration `20260620140000-task-layer.js` RAN on dev** — every project got a **"General" task** adopting its pipelines, runs repointed, `project.pipelines` dropped (verified). Spec `…task-layer-design.md`, plan `…task-layer.md`.
4. **Change 1b — move a personal project into a team.** `POST /projects/:id/transfer/preview` + `/transfer` (`apps/api/src/projects/transfer.service.ts`): bundle = project + its tasks + pipelines + prompts (+ **runs/assets**); **one-way** re-point `workspaceId` in a **Mongo transaction**; sets project **public/all** on landing; **conflict guard** blocks shared deps; **keys never move**. Web **`MoveToTeamModal`** (personal + creator only). **Security-reviewed + fixed**: task-gather now scoped to source ws; runs+assets re-pointed (were leaking personal run history into the team); email-verified gate added.

## ⚠️ Deploy (this session's specifics)
- **Deploy = edit (worktree) → push `feat/members-page:dev` → `git -C <main> merge --ff-only origin/dev` → `pnpm turbo run build` → `pm2 restart lyra-api lyra-web`** (web HMRs; api needs the restart; connectors only if changed).
- **DB migrations** run separately: `cd <main>/apps/api && MONGODB_URI="mongodb://localhost:27017/lyra?replicaSet=rs0" npx migrate-mongo up` (the task-layer migration already ran on dev). Mongo is the docker `lyra-mongo` replica set.

## 🔜 Pending / deferred this session (don't start unless asked)
- **Change 1b follow-ups** (spec'd, unbuilt): **leaf moves** (lone prompt/pipeline); **export-on-leave**; **approval bell notification** (needs a notifications backend); the "duplicate the shared dep" resolution (today the conflict guard just **blocks**).
- **CLAUDE.md is now partly stale** — it predates the Task layer (still says "project = pipelines hub") and Change 1; update it when convenient.
- Prior-entry items still stand: Postiz publish-v2 e2e, per-project channels, permission-normalization, Viewer-role polish (`[[members-area-roadmap]]`, `[[per-project-channels-model]]`).
...
- **All three servers now run from the MAIN tree under pm2** (durability task #4 — was detached worktree procs): `lyra-web` (:5173 vite), `lyra-api` (:3001 dist), `lyra-connectors` (:9100 dist + Deno). pm2 **auto-restarts on crash** and **survives reboot** via a Startup-folder `pm2 resurrect` cmd (`%APPDATA%\…\Startup\lyra-pm2-resurrect.cmd`) + `pm2 save` (dump.pm2). Config: **`ecosystem.config.js` in the main-tree root** (UNTRACKED — has machine paths + the e2e token; the connectors app's `env.PATH` carries the yt-dlp/ffmpeg/deno winget dirs). **Commands:** `pm2 list` / `pm2 logs <name>` / `pm2 restart lyra-connectors`. The old per-port `Start-Process` recipe is retired (the `PORT=9100`-poisons-the-api gotcha is handled by per-app env in the ecosystem file).
- **⚠️ Iteration changed:** since servers serve from the MAIN tree, a code edit goes live by: edit (worktree) → push → **`git -C <main> merge --ff-only origin/dev`** → web HMRs from main-tree source; for api/connectors **rebuild + `pm2 restart lyra-api|lyra-connectors`**. (Or just edit directly in the main tree now that it == origin/dev.) Main-tree `apps/api/.env` is present (gitignored); the worktree copy still exists too.
