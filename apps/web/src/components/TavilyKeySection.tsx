...
// The Tavily search key (powers product-research search). Lives in Settings; the key is
// stored encrypted per workspace and never returned in full.
export function TavilyKeySection() {
  ...
  const load = useCallback(async () => {
    if (!ws) return;
    try {
      setCred(await connectorsApi.tavilyStatus(ws));
    } catch (err) {
  ...
  const save = async () => {
    ...
    setError(null);
    try {
      await connectorsApi.saveCredential(ws, 'tavily', keyInput.trim());
      setKeyInput('');
      await load();
    } catch (err) {
      setError(err instanceof Error ? err.message : t('connectors.error'));
  ...
  return (
    <section className="set-section">
      <div className="set-section-head">
        <h2>{t('connectors.research')}</h2>
        <p>{t('connectors.tavilyKeyHint')}</p>
      </div>
      ...
      <div className="set-keyrow">
        <input
          ...
          type="password"
          placeholder={t('connectors.tavilyKey')}
          value={keyInput}
      ...
      <p className={cred?.connected ? 'cx-badge-ok' : 'cx-badge-off'}>
        ● Tavily {cred?.connected ? `${t('connectors.connected')} ····${cred.last4 ?? ''}` : t('connectors.notConnected')}
      </p>
    </section>
  );
}
