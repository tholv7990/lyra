...
// ScoreAction / CompetitionAction with a mocked LLM returning mid scores for every dimension.
...
describe('research pipeline chain', () => {
  ...
  it('research → gate → score → unit-econ → evaluate → save produces a Product with score/grade/decision', async () => {
    ...
    steps.push(step({ index: 6, kind: StepKind.Action, action: { type: 'evaluate' } as never }));
    steps[6].data = (await new EvaluateAction().execute(run(6))).data;
    steps.push(step({ index: 7, kind: StepKind.Action, action: { type: 'save-product' } as never }));
    const products = { applyResearch: jest.fn().mockResolvedValue({ id: 'prod-1' }) };
    const out = await new SaveProductAction(products as never).execute(run(7));
