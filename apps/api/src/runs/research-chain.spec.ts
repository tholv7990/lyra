...
// ScoreAction with a mocked LLM returning mid scores for every dimension.
...
describe('research pipeline chain', () => {
  ...
  it('research → gate → score → unit-econ → evaluate → save produces a Product with score/grade/decision', async () => {
    ...
    steps.push(step({ index: 2, kind: StepKind.Action, action: { type: 'score' } as never }));
    steps[2].data = (await scoreAction().execute(run(2))).data;
    steps.push(step({ index: 3, kind: StepKind.Action, action: { type: 'unit-econ' } as never }));
    steps[3].data = (await new UnitEconAction().execute(run(3))).data;
