...
import type { EvidenceClaim, SourceRow, UnitEcon, SubScores, ConfidenceGrade, Decision, HardGates, UnitEconInputs } from '@lyra/shared';
...
export class SaveProductAction implements ActionProvider {
  ...
  async execute(ctx: ActionRunContext): Promise<StepRunOutput> {
    const d = ctx.ledger.data as { unitEcon?: UnitEcon; subScores?: SubScores; score?: number; grade?: ConfidenceGrade; decision?: Decision; hardGates?: HardGates };
    if (!ctx.productId) {
