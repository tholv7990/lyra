...
export interface ResearchResult { claims: EvidenceClaim[]; sources: SourceRow[]; rounds: number; llmCalls: number; droppedClaims: number; }
...
const EXTRACT_SYS = 'EXTRACT: From the provided sources ONLY, extract factual claims. Reply ONLY JSON {"claims":[{"statement","value?","kind":"verified|calculated|estimate|assumption","sourceId","geography?","period?","demandSignal?":bool,"purchaseData?":bool}]}. Every claim MUST set sourceId to one of the given source ids. Never assert anything not supported by a source.';
...
export async function runResearch(question: string, deps: ResearchDeps): Promise<ResearchResult> {
  ...
  const ingest = (hits: SearchHit[]): { row: SourceRow; content: string }[] => {
    ...
    for (const h of hits) {
      ...
      sources.push(row); byUrl.set(h.url, row);
      added.push({ row, content: (h.content ?? '').slice(0, 4000) });
    }
    return added;
  };
  ...
  for (let round = 0; round < caps.maxRounds; round++) {
    ...
    if (!capHit() && newRows.length) {
      ...
      for (const c of ex.claims ?? []) {
        if (!c || !byId(sources, c.sourceId)) { droppedClaims++; continue; } // anti-hallucination
        claims.push({
          id: `c${++claimSeq}`,
  ...
  return { claims, sources, rounds, llmCalls, droppedClaims };
}

interface RawClaim { statement?: unknown; value?: number | string; kind?: unknown; sourceId?: string; geography?: string; period?: string; demandSignal?: boolean; purchaseData?: boolean; }
