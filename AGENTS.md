...
(SPA, Vite)                        │
                                   ├─▶ Job worker (BullMQ/Redis, render phase) ─▶ GPT-5.5 · DeepSeek · Codex · Image · Video
...
4. **Brain steps (Codex)** — steps 3/4/5 real via the `StepProvider` interface.
...
- Per-package `AGENTS.md` files are planned (`apps/api`, `apps/web`, `packages/shared`) — create/update them as conventions emerge so rules apply where they're relevant.
